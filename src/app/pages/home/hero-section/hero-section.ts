...
export class HeroSection implements OnDestroy {
  ...
  onMouseMove(e: MouseEvent): void {
    ...
    const MAX = 200; // px
    // Bias toward center: invert direction so mouse-center pulls glows inward
    this.targetAx = -dx * MAX;
    this.targetAy = -dy * MAX;
    this.targetBx =  dx * MAX;
    this.targetBy =  dy * MAX;
  ...
  onMouseDown(): void {
    this.el.nativeElement.classList.add('mouse-down');
  }

  onMouseUp(): void {
    this.el.nativeElement.classList.remove('mouse-down');
  }

  private loop(): void {
    const lerp = (a: number, b: number, t: number) => a + (b - a) * t;
    const SPEED = 0.1; // 0 = never catches, 1 = instant
